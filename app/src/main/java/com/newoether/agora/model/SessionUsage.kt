...
/**
 * One per-request usage record, durably stored for the session usage dashboard.
 * Nullable numeric fields mean "the provider did not supply this value" — never zero.
 * Cache breakdown presence is tracked by [cacheDetailsStatus] so partial provider
 * reporting is distinguishable from a genuine zero-cache-hit request.
 */
@Serializable
...
data class ConversationUsageSummary(
    ...
    /** True when every provider-reported record included a cache breakdown (hit ratio is meaningful). */
