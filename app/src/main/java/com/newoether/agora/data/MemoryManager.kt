...
class MemoryManager(context: Context) {
    ...
    @Synchronized fun updateActiveMemory(content: String, mode: String = "replace", oldString: String? = null, newString: String? = null): String {
        ...
        val next = when (mode) {
            ...
            "patch" -> {
                ...
                require(old.countOccurrences(oldString) == 1) { "old_string must match exactly once in active memory" }
                old.replace(oldString, newString ?: "")
            }
    ...
    @Synchronized private fun loadMeta(): MutableMap<String, String> =
        if (metaFile.exists()) runCatching { json.decodeFromString<MutableMap<String, String>>(metaFile.readText()) }.getOrElse { mutableMapOf() } else mutableMapOf()
    ...
    @Synchronized fun saveMetaJson(value: String) { json.parseToJsonElement(value); atomicWrite(metaFile, value) }
    ...
    @Synchronized fun editFile(name: String, content: String? = null, newName: String? = null, description: String? = null, oldString: String? = null, newString: String? = null): String {
        val f = resolveFile(name); require(f.exists()) { "File not found: $name" }; require(!(content != null && oldString != null)) { "content and old_string are mutually exclusive" }
        var target = f
        if (oldString != null) { val before = f.readText(); require(before.countOccurrences(oldString) == 1) { "old_string must match exactly once in ${f.name}" }; atomicWrite(f, before.replace(oldString, newString ?: "")) } else if (content != null) atomicWrite(f, content)
        if (newName != null && newName != name) { target = resolveFile(newName); require(!target.exists()) { "Target file already exists: ${target.name}" }; require(f.renameTo(target)) { "Could not rename ${f.name}" }; val m = loadMeta(); m.remove(f.name)?.let { m[target.name] = it }; saveMeta(m) }
        if (description != null) { val m = loadMeta(); if (description.isBlank()) m.remove(target.name) else m[target.name] = description; saveMeta(m) }
    ...
    private fun atomicWrite(file: File, text: String) { val tmp = File(file.parentFile, file.name + ".tmp-${Thread.currentThread().id}"); FileOutputStream(tmp).use { it.write(text.toByteArray(Charsets.UTF_8)); it.fd.sync() }; if (!tmp.renameTo(file)) { if (file.exists()) file.delete(); require(tmp.renameTo(file)) { tmp.delete(); error("Could not atomically write ${file.name}") } } }
